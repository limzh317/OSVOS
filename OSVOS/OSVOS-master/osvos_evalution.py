...
class DAVIS2017(Dataset):
    ...
    def __init__(self, split, db_root_dir=None, transforms=None, file= None):
        ...
        images = os.listdir(os.path.join(db_root_dir, frame_image, fname, file))
        images.sort()
        # print(images)

        images_path = list(map(lambda x: os.path.join(db_root_dir, frame_image, fname, file, x), images))
...
class DavisParent:
    ...
    def __init__(self, model_name):
        ...
        self.evalution = True
        self.saveRoot = os.path.join('OSVOS/data/Output/Unlabel/test/', self.model_name, self.split, file, 'Prediction/')
    ...
    def vis_plot(self, vis_mask, vis_gt, vis_image, file, val_ind):
        #ipdb.set_trace()
        plt.axis('off')
        ...
        #saveRoot = os.path.join('OSVOS/data/Output/Unlabeled/test/', self.model_name, self.split, file, 'Prediction/')
        if not os.path.exists(self.saveRoot):
            os.makedirs(self.saveRoot)
        plt.savefig(self.saveRoot + '/' +  str(val_ind) + '.png')
    ...
    def evalution_parent_epoch(self, epoch, file):
        ...
        with torch.no_grad():
            ...
            for batch_idx, sample in enumerate(self.val_parent_loader):
                ...
                #ipdb.set_trace()
                if self.use_visdom:
                    for img_idx in range(raw_img.shape[0]):
                        vis_image = raw_img[img_idx]
                        vis_mask = bin_mask[img_idx].data.cpu().numpy() * 50
                        ...
                        #self.vis_plot(vis_mask, vis_gt, vis_image, file, val_ind)
                        val_ind += 1
        ...
        import scipy.io
        #saveRoot = os.path.join('OSVOS/data/Output/Unlabeled/test/', self.model_name, self.split, file)
        # if not os.path.exists(saveRoot):
        #     os.makedirs(saveRoot)
        # #ipdb.set_trace()
        scipy.io.savemat(self.saveRoot + '/' + file + '.mat', {'bin_mask_file': bin_mask_file.data.cpu().numpy()})
    ...
    def Get_green_list(self):
        root = '/p300/dataset/data/images_new_/COLOR/GREEN/'
        green_list = os.listdir(root)
        return green_list
    ...
    def main(self):
        ...
        unlabelset = self.Get_green_list()
        ...
        if self.split == 'val':
            ...
            raise Exception('Only support val, test!')
        ...
        for file in set:
            ...
            acc_val_file, dice_coeff_val_file = self.evalution_parent_epoch(epoch=epoch,file = file) #each file
            #ipdb.set_trace()
